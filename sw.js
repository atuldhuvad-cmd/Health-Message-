...
const CACHE='health-infographic-studio-2.4-final-v1';
const CORE=[
  './',
  './index.html',
...
self.addEventListener('fetch',event=>{
  ...
  if(mustRefresh){event.respondWith(fetch(event.request).then(response=>{if(response.ok&&sameOrigin)caches.open(CACHE).then(cache=>cache.put(event.request,response.clone())).catch(()=>{});return response}).catch(()=>caches.match(event.request).then(cached=>cached||caches.match('./Health_Infographic_Studio_2_4_FINAL_RELEASE.html')||caches.match('./index.html'))));return}
