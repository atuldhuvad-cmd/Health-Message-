...
const CACHE='health-infographic-studio-2.4-calendar-fixed-v1';
const CORE=[
  './',
  './Health_Infographic_Studio_2_4_2026_CALENDAR_FIXED.html',
...
self.addEventListener('fetch',event=>{
 ...
 if(mustRefresh){event.respondWith(fetch(event.request).then(response=>{if(response.ok&&sameOrigin)caches.open(CACHE).then(cache=>cache.put(event.request,response.clone())).catch(()=>{});return response}).catch(()=>caches.match(event.request).then(cached=>cached||caches.match('./Health_Infographic_Studio_2_4_2026_CALENDAR_FIXED.html'))));return}
