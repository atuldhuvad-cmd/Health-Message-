...
const CACHE='health-infographic-studio-2.3-final-empty-webhook-protection-v2';
const CORE=['./','./index.html','./Health_Infographic_Studio_2_3_FINAL_RELEASE.html','./manifest.webmanifest'];
const ARTWORK=[
...
self.addEventListener('fetch',event=>{
  ...
  if(mustRefresh){event.respondWith(fetch(event.request).then(response=>{if(response.ok&&sameOrigin)caches.open(CACHE).then(cache=>cache.put(event.request,response.clone())).catch(()=>{});return response}).catch(()=>caches.match(event.request).then(cached=>cached||caches.match('./index.html'))));return}
